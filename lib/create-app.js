...
import { DataPipeEnabled } from './datapipe-config.js';
...
// Function to check and setup notifications
function setupNotifications(userId) {
    // Make sure we're in Cordova and plugin is loaded
...
function scheduleNotifications(userId) {
    ...
    cordova.plugins.notification.local.cancelAll(function() {
        ...
        cordova.plugins.notification.local.schedule(testNotif);
        console.log('Test notification scheduled for 10 seconds from now');
        
        // Schedule recurring notifications every minute
        const recurringNotif = {
            id: 2,
        ...
        cordova.plugins.notification.local.schedule(recurringNotif);
        console.log('Recurring notification scheduled for every minute');
        
        // Schedule multiple notifications for the next few minutes
        for (let i = 1; i <= 5; i++) {
            const notif = {
                id: 100 + i,
                title: \`Reminder \${i}\`,
                text: \`This is notification \${i} - scheduled for \${i} minute(s) from now\`,
                trigger: { at: new Date(Date.now() + (i * 60000)) },
                foreground: true,
                vibrate: true,
                icon: 'res://icon',
                smallIcon: 'res://icon'
        ...
        console.log('Scheduled 5 notifications for the next 5 minutes');
...
async function onDeviceReady() {
    ...
    // Add a notification test screen
    timeline.push({
        type: jsPsychHtmlButtonResponse,
        stimulus: \`
...
<widget id="com.example.${projectName.replace(/[^a-zA-Z0-9]/g, '')}" version="1.0.0" xmlns="http://www.w3.org/ns/widgets" xmlns:cdv="http://cordova.apache.org/ns/1.0">
    ...
    try {
      execSync('npx cordova plugin add cordova-plugin-local-notification', { stdio: 'inherit' });
    ...
    try {
      execSync('npx cordova plugin add cordova-plugin-wkwebview-file-xhr', { stdio: 'inherit '});
